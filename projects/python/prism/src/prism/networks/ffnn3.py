...
class TanhNet(nn.Module):
    def __init__(self, n_inputs=2, n_hidden=(3, 3, 3)):
        super(TanhNet, self).__init__()
        self.fc1 = nn.Linear(n_inputs, n_hidden[0])
...
class FFNN3:
    ...
    def get_w3(self):
        if self.device == "cpu":
            return self.net.fc3.weight.detach().numpy()
        else:
            return self.net.fc3.weight.cpu().detach().numpy()

    def get_b3(self):
        if self.device == "cpu":
            return self.net.fc3.bias.detach().numpy().reshape(-1, 1)
        else:
            return self.net.fc3.bias.cpu().detach().numpy().reshape(-1, 1)

    def get_w4(self):
        if self.device == "cpu":
            return self.net.fc4.weight.detach().numpy()
        else:
            return self.net.fc4.weight.cpu().detach().numpy()

    def get_b4(self):
        if self.device == "cpu":
            return self.net.fc4.bias.detach().numpy().reshape(-1, 1)
        else:
            return self.net.fc4.bias.cpu().detach().numpy().reshape(-1, 1)
...
def visual_test_save_load():
    ...
    # Save the model
    path = "./test_models/test_ffnn3"
    filename = ffnn.save(path)

    # Load the model
    ffnn2 = FFNN3.load(filename)
...
def visual_test_numpy_equivalence_relu():
    ...
    v = ffnn.get_w1() @ x.T + ffnn.get_b1()
    v = ffnn.get_w2() @ relu(v) + ffnn.get_b2()
    v = ffnn.get_w3() @ relu(v) + ffnn.get_b3()
    y_relu_eq = ffnn.get_w4() @ relu(v) + ffnn.get_b4()
...
def visual_test_numpy_equivalence_tanh():
    ...
    v = ffnn.get_w1() @ x.T + ffnn.get_b1()
    v = ffnn.get_w2() @ np.tanh(v) + ffnn.get_b2()
    v = ffnn.get_w3() @ np.tanh(v) + ffnn.get_b3()
    y_tanh_eq = ffnn.get_w4() @ np.tanh(v) + ffnn.get_b4()

    _, axs = plt.subplots(figsize=(8, 6))
    axs.plot(t, y_tanh_eq.reshape(-1), linewidth=8)
    axs.plot(t, y_net, linewidth=2)
    axs.legend(("Network", "Equation"))
