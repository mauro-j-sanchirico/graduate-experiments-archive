...
class TanhNet(nn.Module):
    def __init__(self, n_inputs=2, n_hidden=(3, 3, 3)):
        ...
        for idx in range(len(n_hidden) - 1):
            self.layers.append(nn.Linear(n_hidden[idx], n_hidden[idx + 1]))
        self.layers.append(nn.Linear(n_hidden[-1], 1))
...
class DNN:
    ...
    def __init__(
        ...
        """Initializes the DNN wrapper class"""
    ...
    # Getters
    def get_w(self, layer_n):
        if self.device == "cpu":
            return self.net.layers[layer_n].weight.detach().numpy()
        else:
            return self.net.layers[layer_n].weight.cpu().detach().numpy()

    def get_b(self, layer_n):
        if self.device == "cpu":
            return (
                self.net.layers[layer_n].bias.detach().numpy().reshape(-1, 1)
            )
        else:
            return (
                self.net.layers[layer_n]
                .bias.cpu()
                .detach()
                .numpy()
                .reshape(-1, 1)
...
def visual_test_save_load():
    ...
    # Save the model
    path = "./test_models/test_dnn3"
    filename = dnn.save(path)

    # Load the model
    dnn2 = DNN.load(filename)
...
def visual_test_numpy_equivalence_relu():
    ...
    # Infer the function with a numpy equation
    x = x.T
    n_layers = len(dnn.net.layers)

    for idx in range(n_layers - 1):
        v = dnn.get_w(idx) @ x + dnn.get_b(idx)
        x = relu(v)

    y_eq = dnn.get_w(n_layers - 1) @ x + dnn.get_b(n_layers - 1)

    _, axs = plt.subplots(figsize=(8, 6))
    axs.plot(t, y_eq.reshape(-1), linewidth=8)
    axs.plot(t, y_net, linewidth=2)
