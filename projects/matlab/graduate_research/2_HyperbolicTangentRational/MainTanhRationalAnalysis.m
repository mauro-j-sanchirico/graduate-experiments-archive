%% Analysis of the Rational Expansion for Tanh
%% Setup
...
%
% Here we plot a rational approximation of tanh(x) with a varying number
% of terms.  The function ApproxTanhRational uses an n-term rational
% series approximation of tanh(x).
%
...
for idx = 1:length(n)
    y_approx{idx} = ApproxTanhRational(x,n(idx));
...
xlabel('x');
ylabel('y');
title('Rational Approximations of tanh(x)');
...
for idx = 1:length(alpha)
    y_approx{idx} = ApproxTanhRational(alpha(idx)*x, n);
...
title('Rational Approximations of tanh({\alpha}sin(x))');
legend(legend_strs, 'location', 'southeast');
...
n_max = 50;
AnalyzeApproxTanhRationalError(n_max, alpha);
