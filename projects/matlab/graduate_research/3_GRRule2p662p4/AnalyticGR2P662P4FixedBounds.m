...
% Assumes that the lower bound is 0 and the upper bound is 2*pi/b and
% makes the cooresponding simplifications to the formulas.
%
...
%
function [definite_integral] = AnalyticGR2P662P4FixedBounds(a, b, m)
definite_integral = 0;

e0 = 1/2^(2*m);
e1 = exp(a*2*pi/b)/2^(2*m);

for k = 0:m
   
    q = (-1)^k/(a^2 + (2*k+1)^2*b^2);
    c = nchoosek(2*m+1, m-k);
    f1 = a*sin((2*k+1)*2*pi) - (2*k+1)*b*cos((2*k+1)*2*pi);
    f0 = (2*k+1)*b;
