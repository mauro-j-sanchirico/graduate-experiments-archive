...
for idx = 1:analysis_params.max_optimization_attempts
    ...
    low_bound = ...
        -ones(size(mlp_init_param_vector))*analysis_params.max_weight;
    high_bound = ...
        ones(size(mlp_init_param_vector))*analysis_params.max_weight;

    % Define optimizer options
    %
    % Note that options.Algorithm = 'levenberg-marquardt'; can only be 
    % used if there are no bounds.  We leave it off here since we want to
    % bound the optimization inside the expansion's region of convergence.
    ...
    found_mlp_param_vector = lsqnonlin( ...
        optim_func, mlp_init_param_vector, ...
    ...
    % Check the error to see if we need to repeat
    y_found_mlp = EvalMLP(mlp_params, x_stimulus);
    e = mse(y_found_mlp, y_measured);
    
    if e < analysis_params.repeat_optimization_tolerance
        %fprintf('Found satisfactory replication solution.\n');
        found_mlp_params = mlp_params;
        break;
    elseif e < e_best
        %fprintf('Found improved replication solution.\n');
        found_mlp_params = mlp_params;
    end
