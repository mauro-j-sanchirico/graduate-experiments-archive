...
addpath('../Math');
addpath('../Math/KappaFourierSeries/');
...
% Gamma sweep
d_gamma = 0.05;
gamma_sweep_up = 0:d_gamma:6;
gamma_sweep_down = 6:-d_gamma:0;
...
gif_file_name = sprintf( ...
    '../Animations/KappaFourierSeriesAnimation_L%i.gif', L);
...
for idx = 1:length(gamma_sweep)
    ...
    [a_fourier_kappa, b_fourier_kappa, ...
     c_fourier_kappa, n_fourier_kappa] = ...
        ComputeKappaFourierSeries(gamma, num_fourier_harmonics, L);
    ...
    c_error = abs(c_fourier_true - c_fourier_kappa);
    ...
    % Plot Fourier
    subplot(246)
    bh = bar(n_fourier_true, [c_fourier_true' c_fourier_kappa']);
    ...
    hold on;
    PlotFourierSeries( ...
        t, a_fourier_kappa, b_fourier_kappa, n_fourier_kappa, 'r');
    xlim([min(psi) max(psi)]);
    ylim([-1.5 1.5]);
    xlabel('$$\psi$$');
    ylabel('$$\hat{y}(\psi)$$')
